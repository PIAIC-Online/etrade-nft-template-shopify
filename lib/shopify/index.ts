...
export async function getHeroSection() {
  const query = `query HeroSection {
    metaobject(handle: {handle: "sale", type: "hero_section"}) {
      fields {
        key
        value
        reference {
...
export async function getSliderSection() {
  const query = `query HeroSection {
    metaobject(handle: {handle: "slider-herosection", type: "slider"}) {
      field(key: "slides") {
        key
        references(first: 10) {
          nodes {
  ...
  const data = res.body.data.metaobject.field.references.nodes.map((item: any) => {
    ...
    const returnSliderObjects: any ={};
    item.fields.forEach((item: any) => {
      returnSliderObjects['key'] = item.key;
      if(item.reference !== null){
        returnSliderObjects['imageURL'] = item.reference.image.url;
      } else {
        returnSliderObjects['value'] = item.value;

      }
      returnSliderObjects['price'] = item.price;
      });
    return returnSliderObjects;
  });
